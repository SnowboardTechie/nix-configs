...
| hindsight | 8888 (API), 9999 (Control Plane), 9998 (Control Plane header-normalization proxy), 5433 (PostgreSQL) — loopback; 9443/9444 Tailscale Serve HTTPS | uv-locked venv `hindsight-api` + npm-locked Control Plane + Caddy normalization proxy + nixpkgs postgresql_17+pgvector | Always-on + 6-hourly backup + monthly restore test | Named per-user instances (`services.hindsight.instances.bryan`). Locks in `hindsight-env/` (uv.lock + package-lock.json); service start applies them exactly (`uv sync --locked`, `npm ci`). The `9998` proxy prevents Hindsight's Next.js locale middleware from treating its plain-HTTP localhost listener as HTTPS behind Tailscale Serve. Secrets read at exec time from `~/.secrets/hindsight-<name>/` — never in plists/derivations. Backups age-encrypted to declared public recipient; tiered retention 48h/14d/4w + pre-upgrade via `hindsight-<name>-backup-now pre-upgrade`. PostgreSQL major upgrades are explicit migrations, never routine rebuilds. |
